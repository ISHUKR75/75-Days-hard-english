...
// Filler word banks that plug into the 8 self-introduction sentence patterns below,
// so the generator produces genuinely varied (not just repeated) real sentences.
const NAMES = [
  ['आरव', 'Aarav'], ['दिशा', 'Disha'], ['रोहन', 'Rohan'], ['प्रिया', 'Priya'], ['विकास', 'Vikas'], ['नेहा', 'Neha'],
  ['अमन', 'Aman'], ['काव्या', 'Kavya'], ['सूरज', 'Suraj'], ['इशा', 'Isha'], ['करण', 'Karan'], ['अंजलि', 'Anjali'],
  ['राहुल', 'Rahul'], ['श्रुति', 'Shruti'], ['विवेक', 'Vivek'], ['पूजा', 'Pooja'], ['अर्जुन', 'Arjun'], ['सिमरन', 'Simran'],
];
const CITIES = [
  ['दिल्ली', 'Delhi'], ['मुंबई', 'Mumbai'], ['बेंगलुरु', 'Bengaluru'], ['पुणे', 'Pune'], ['जयपुर', 'Jaipur'], ['लखनऊ', 'Lucknow'],
  ['चेन्नई', 'Chennai'], ['कोलकाता', 'Kolkata'], ['अहमदाबाद', 'Ahmedabad'], ['सूरत', 'Surat'], ['इंदौर', 'Indore'], ['भोपाल', 'Bhopal'],
];
const PROFESSIONS = [
  ['छात्र', 'student'], ['इंजीनियर', 'engineer'], ['टीचर', 'teacher'], ['डॉक्टर', 'doctor'], ['मैनेजर', 'manager'],
  ['डिज़ाइनर', 'designer'], ['अकाउंटेंट', 'accountant'], ['नर्स', 'nurse'], ['वकील', 'lawyer'], ['किसान', 'farmer'],
  ['बिज़नेसमैन', 'businessman'], ['पत्रकार', 'journalist'],
];
const HOBBIES = [
  ['अंग्रेज़ी सीखना', 'learning English'], ['किताबें पढ़ना', 'reading books'], ['क्रिकेट खेलना', 'playing cricket'],
  ['गाने सुनना', 'listening to music'], ['यात्रा करना', 'traveling'], ['खाना बनाना', 'cooking'],
  ['फ़ोटो खींचना', 'taking photos'], ['नई भाषाएँ सीखना', 'learning new languages'], ['योग करना', 'doing yoga'],
  ['पेंटिंग करना', 'painting'], ['बागवानी करना', 'gardening'], ['लिखना', 'writing'],
];
const CAREER_GOALS = [
  ['अपने करियर में आगे बढ़ना', 'grow in my career'], ['मैनेजर बनना', 'become a manager'],
  ['अपना बिज़नेस शुरू करना', 'start my own business'], ['विदेश में काम करना', 'work abroad'],
  ['नई स्किल्स सीखना', 'learn new skills'], ['एक बेहतर लीडर बनना', 'become a better leader'],
  ['अपनी टीम को बढ़ाना', 'grow my team'], ['एक सफल उद्यमी बनना', 'become a successful entrepreneur'],
];
const GREETINGS = [
  ['आपसे मिलकर खुशी हुई।', 'Nice to meet you.'], ['आपसे बात करके अच्छा लगा।', 'It was nice talking to you.'],
  ['मुझे आपसे मिलकर खुशी हो रही है।', "I'm glad to meet you."], ['आपका स्वागत है।', 'Welcome!'],
  ['फिर मिलते हैं।', 'See you again.'], ['आपका दिन शुभ हो।', 'Have a nice day.'],
];
const FAMILY_SIZES_HI = ['दो', 'तीन', 'चार', 'पाँच', 'छह', 'सात'];
const FAMILY_SIZES_EN = ['two', 'three', 'four', 'five', 'six', 'seven'];

function selfIntroQuestion(dayNum, index, topic) {
  const name = pick(NAMES, index);
  const city = pick(CITIES, index + 1);
  const profession = pick(PROFESSIONS, index + 2);
  const hobby = pick(HOBBIES, index + 3);
  const familyIdx = index % FAMILY_SIZES_HI.length;
  const goal = pick(CAREER_GOALS, index + 4);
  const years = 1 + (index % 9);
  const greeting = pick(GREETINGS, index + 5);
  const patterns = [
    [`मेरा नाम ${name[0]} है।`, `My name is ${name[1]}.`, 'My name is + name'],
    [`मैं ${city[0]} से हूँ।`, `I am from ${city[1]}.`, 'I am from + place'],
    [`मैं एक ${profession[0]} हूँ।`, `I am a ${profession[1]}.`, 'I am a/an + profession'],
    [`मुझे ${hobby[0]} पसंद है।`, `I like ${hobby[1]}.`, 'I like + verb-ing'],
    [`मेरे परिवार में ${FAMILY_SIZES_HI[familyIdx]} लोग हैं।`, `There are ${FAMILY_SIZES_EN[familyIdx]} people in my family.`, 'There are + number + people'],
    [`मैं ${goal[0]} चाहता हूँ।`, `I want to ${goal[1]}.`, 'I want to + verb'],
    [`मेरे पास ${years} साल का अनुभव है।`, `I have ${years} year${years > 1 ? 's' : ''} of experience.`, 'I have + years of experience'],
    [greeting[0], greeting[1], 'Greeting phrase'],
  ];
...
// ============================================================
// De-duplicates by hindi+english so real (handcrafted or JSON-authored) content
// is never dropped. IMPORTANT: real content is never truncated below the
// target — it is only PADDED with generated filler when it falls short. This
// means a day with 1000 real curated questions correctly shows all 1000
// instead of being cut down to a smaller generic target.
export function ensurePracticeTarget(dayNum, existing = [], targetCount = GENERATED_CONTENT_TARGETS.practiceQuestions) {
  ...
  if (dedupedExisting.length >= targetCount) return dedupedExisting;
  const needed = targetCount - dedupedExisting.length;
  const generated = generatePracticeQuestionsForDay(dayNum, needed + 100).filter((item) => {
    const key = `${item.hindi}|${item.english}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
  return [...dedupedExisting, ...generated].slice(0, targetCount);
}

// Same non-truncating rule as above for test questions.
export function ensureTestTarget(dayNum, existing = [], targetCount = GENERATED_CONTENT_TARGETS.testQuestions) {
  ...
  if (normalizedExisting.length >= targetCount) return normalizedExisting;
  const needed = targetCount - normalizedExisting.length;
  const generated = generateTestQuestionsForDay(dayNum, needed + 100);
  return [...normalizedExisting, ...generated].slice(0, targetCount);
}
