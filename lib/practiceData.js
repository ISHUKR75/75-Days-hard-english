...
export function getQuestionsForDay(dayNum) {
  if (ALL_QUESTIONS[dayNum]) return ALL_QUESTIONS[dayNum]; // Days 1-30
  if (EXTENDED_QUESTIONS[dayNum]) return EXTENDED_QUESTIONS[dayNum]; // Days 31-75 real questions
  const topic = DAYS_75_TOPICS.find(t => t.day === dayNum);
  const title = topic ? topic.title : `Day ${dayNum} Topic`;
  return generateQuestionsForDay(dayNum, title); // Fallback generator
}
