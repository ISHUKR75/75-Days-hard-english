...
import { getTopicByDay } from './topics';

// Real, handcrafted per-day vocabulary banks authored in data/challenge/day-XX/vocabulary.json.
// These are higher quality than the generic filler below and are always shown first, in full.
// Add more entries here as additional days get a real vocabulary.json authored.
import day01VocabJson from '../data/challenge/day-01/vocabulary.json';

const REAL_VOCAB_JSON_BY_DAY = {
  1: day01VocabJson.words,
};

function normalizeRealVocabWord(raw, dayNum, topicTitle) {
  const level = raw.cefrLevel || 'A1';
  return {
    ...
    meaning: raw.simpleMeaning || '',
    example: raw.sentences?.daily || `Use "${raw.word}" in a sentence.`,
    officeEx: raw.sentences?.office || raw.sentences?.business || '',
    category: topicTitle || 'Vocabulary',
    day: dayNum,
    level,
    difficulty: (level === 'A0' || level === 'A1') ? 'easy' : (level === 'A2' || level === 'B1') ? 'medium' : 'hard',
    verbForms: raw.verbForms || null,
    synonyms: raw.synonyms || [],
    antonyms: raw.antonyms || [],
    usageNote: raw.usageNote || '',
    source: 'handcrafted',
  };
}
...
// Returns the full vocabulary list for a given day. Real, handcrafted words
// (authored in data/challenge/day-XX/vocabulary.json) always come first and
// are NEVER truncated below the target; auto-generated words only fill the
// remainder so every day still reaches a rich, real vocabulary bank.
export function getVocabularyForDay(dayNum, targetCount = 500) {
  const topic = getTopicByDay(dayNum);
  const real = (REAL_VOCAB_JSON_BY_DAY[dayNum] || []).map((w) => normalizeRealVocabWord(w, dayNum, topic?.title));
  if (real.length >= targetCount) return real;
  const seen = new Set(real.map((w) => w.word.toLowerCase()));
  const generated = generateVocabularyForDay(dayNum, (targetCount - real.length) * 2)
    .filter((w) => !seen.has((w.word || '').toLowerCase()));
  return [...real, ...generated].slice(0, targetCount);
}
